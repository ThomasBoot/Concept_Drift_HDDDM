...
from HDDDM_main_approach import HDDDM
...
categorical_variables = ['y']   # Defines which variables are categorical
...
Dataset = 'Datasets/SEA_Abrupt_high.csv'
...
if to_keep == "all":
    detector = HDDDM(distance, categorical_variables + numerical_cols, gamma)
else:
    detector = HDDDM(distance, to_keep, gamma)
...
def visualize_drift(acc, drift, warn):
    ...
    ax.legend()
    plt.title("Hyperplane - No Drift")
    plt.show()
    fig.savefig('Figures/Hyperplane_none.png')
...
def visualize_magnitude(magn):
    ...
    plt.title("Hyperplane - No Drift")
    plt.show()
    fig.savefig('Figures/Hyperplane_none_magn.png')
